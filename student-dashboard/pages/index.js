...
const INITIAL_SAVED_PEERS = [
    { id: 'CS-2024-001', name: 'Alice Student', color: 'bg-blue-100 text-blue-700' },
    { id: 'CS-2024-002', name: 'Bob Peer', color: 'bg-fuchsia-100 text-fuchsia-700' },
];

const VENDOR_LIST = [
    { id: 'CAFE01', name: 'Campus Canteen', color: 'bg-orange-100 text-orange-700' },
    { id: 'LIB01', name: 'Library Cafe', color: 'bg-cyan-100 text-cyan-700' },
];
...
export default function App() {
    ...
    const API_BASE_URL = 'http://localhost:8000'; // Update this when going live
    ...
    const handlePayment = async (entityId, amount, type, extraData = {}) => {
        ...
        try {
            // Map to exact APIs.md endpoints
            if (type === 'P2P') {
                ...
                await apiRequest(`/student/pay-fine/${entityId}`, 'POST'); // entityId is fine_id
        ...
        const newTransaction = {
            ...
            amount: type === 'FINE' || type === 'SUBSCRIPTION' || type === 'VENDOR' ? -numAmount : -numAmount,
            date: 'Just now',
            status: 'COMPLETED'
        };
    ...
    const LoginScreen = () => {
        ...
        const handleAuth = async (e) => {
            ...
            try {
                ...
                setUser({ email: email || 'student@campusflow.com', token: 'mock' });
            }
            setLoading(false);
            navigate('Dashboard');
        };
                ...
                <div className="px-6 space-y-4">
                    ...
                    <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-600 shadow-lg shadow-blue-500/30 p-6 rounded-[24px] relative overflow-hidden">
                        ...
                        <div className="pt-4 border-t border-white/20 flex justify-between items-center relative z-10">
                            <p className="text-xs font-medium text-blue-100">ID: STU-2026-042</p>
                    ...
                    {/* Bento Block 2: 2x2 Grid Actions */}
                    ...
                    {/* Bento Block 3: AI Insight Strip */}
                    <button
                        onClick={() => {
            ...
            <MobileWrapper title={isVendor ? "Pay Vendor" : "Send Money"} showBack>
                <div className="px-6 flex flex-col h-full space-y-4">
                    ...
                    <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm flex-1">
                        ...
                        {addingPeer && !isVendor && (
                            <div className="p-4 bg-slate-50 rounded-[16px] border border-slate-100 space-y-3 mb-4 animate-in fade-in">
                        ...
                        <input
                            type="text"
                            placeholder={isVendor ? "Enter Vendor ID" : "Enter Student ID"}
                            value={target}
                            onChange={e => { setTarget(e.target.value.toUpperCase()); setSelectedEntity(null); }}
